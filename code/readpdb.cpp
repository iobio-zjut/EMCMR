...
//    Model proteins;
    ...
    while(!ReadFile.eof())
    {
        ...
        newatom.chara=atomline.substr(16,1);
            ...
            newatom.xyzVect.push_back(strtof(atomline.substr(38,8).c_str(),0));
            newatom.xyzVect.push_back(strtof(atomline.substr(46,8).c_str(),0));
            if(atomline.length()>54)
            {
                newatom.str_end = atomline.substr(54,atomline.length()-1);
            }
            if(atomline.size()>60){
                newatom.tempfac=(strtof(atomline.substr(60,6).c_str(),0));
            }
...
void readpdbstructurey(string filename,Model &proteins)
{
    string atomline;
    Atom newatom;
    Residue newresidue;
...
void readssfile(string filename, vector<press> &presstype)
{
    ...
    while(!ReadFile.eof())
    {
        ...
        {
            press pret;
            string sst= (atomline.substr(0,6));
            while ( !sst.empty() && sst[0] == ' ' ) sst = sst.substr(1,sst.size()-1);
            while ( !sst.empty() && sst[sst.size()-1] == ' ' ) sst = sst.substr(0,sst.size()-1);
            pret.ss_ord = (stoi(sst));
//            pret.ss_ord = (stoi(atomline.substr(0,6)));
            sst= (atomline.substr(6,7));
            while ( !sst.empty() && sst[0] == ' ' ) sst = sst.substr(1,sst.size()-1);
            while ( !sst.empty() && sst[sst.size()-1] == ' ' ) sst = sst.substr(0,sst.size()-1);
            pret.res_name = (sst);
//            pret.res_name = (atomline.substr(6,7));
//          cout<<"1"<<atomline.substr(6,7)<<2<<endl;
            sst = (atomline.substr(13,4));
            while ( !sst.empty() && sst[0] == ' ' ) sst = sst.substr(1,sst.size()-1);
            while ( !sst.empty() && sst[sst.size()-1] == ' ' ) sst = sst.substr(0,sst.size()-1);
            char srty = sst[0];
            itosse(srty);
    ...
    for(int i=0;i<protein.chains.size();i++)
    {
        ...
        for(int j=0;j<Chanx.residues.size();j++)
        {
            ...
            for(int k=0;k<Resdx.atoms.size();k++)
            {
                ...
                //      outfile<<setw(5)<<protein.chains[i].residues[j].atoms[k].serial;
                outfile<<setw(5)<<n;
//                outfile<<setw(5)<<Atmx.serial;
                outfile<<setw(5)<<Atmx.atona;
                outfile<<setw(1)<<Atmx.chara;
                outfile<<setw(3)<<StoAA(Resdx.resname);
                ...
                string frg= Atmx.str_end;
//                if(frg.length()>0)
//                {
//                    outfile<<Atmx.str_end;
//                }
    //                      outfile<<setw(6)<<setiosflags(ios::fixed)<<setprecision(2)<<coordx.occ[i];
    //                      outfile<<setw(6)<<setiosflags(ios::fixed)<<setprecision(2)<<coordx.tempfac[i];
    //                      outfile<<setw(12)<<coordx.ele[i];
...
void writePDBStructurek(const char *filename,Model &protein,string outname)
{
        ofstream outfile;
 //       string fin_name = "fin.pdb";
        string fin_name = outname;
...
void writePDBStructurez(const char *filename,Model &protein,string outname)
{
        ofstream outfile;
 //       string fin_name = "fin.pdb";
        string fin_name = outname;
