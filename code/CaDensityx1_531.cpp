...
ElectronDensity::init() {
	...
	ATOM_MASK_PADDING = pATOM_MASK_PADDING;
	CA_MASK = pCA_MASK;
	WINDOW_ = pWINDOW_;
...
float ElectronDensity::matchposex(vector<poseCoord> PB)
{
    ...
    rhoC.dimension(density.u1() , density.u2() , density.u3(),0.0);
    inv_rho_mask0.dimension(density.u1() , density.u2() , density.u3(),1.0);
...
vector<float> ElectronDensity::matchposex_t(vector<poseCoord> PB, int seq_num)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    vector<float> CC_res3;
    string elt_i;   
    int pnum = seq_num; 
    vector<float> atm_idx;
    ObjexxFCL::FArray3D< float > rhoC;
    ...
    vector<vector<float>> seq_box(seq_num,vector<float>(6,0.0)); 
...
//            if ( del_ijx[2] < -0.5 ) del_ijx[2]+=1.0;                    
            ...
            MatrixTimesTransVector(f2c,del_ijx,frac_tmpz);
            if(square_len(frac_tmpz)> (ATOM_MASK_PADDING+CA_MASK)*(ATOM_MASK_PADDING+CA_MASK) ) continue;  
            if(float(z)>bond2max) bond2max = float(z);  
            if(float(z)<bond2min) bond2min = float(z);                                    
...
//                if ( del_ijx[1] < -0.5 ) del_ijx[1]+=1.0;                        
                ...
                MatrixTimesTransVector(f2c,del_ijx,frac_tmpy);
                if(square_len(frac_tmpy)> (ATOM_MASK_PADDING+CA_MASK)*(ATOM_MASK_PADDING+CA_MASK) ) continue;   
                if(float(y)>bond1max) bond1max = float(y);  
                if(float(y)<bond1min) bond1min = float(y);                                     
...
//                            cout<<"MASK: "<<theDensityMap.ATOM_MASK<<endl;
                ...
                //    float sigmoid_msk = exp( sqrt(d2) - (theDensityMap.CA_MASK)  );
                    ...
                    if(float(x)>bond0max) bond0max = float(x);
                    if(float(x)<bond0min) bond0min = float(x);                    
...
//                            }
        ...
        seq_box[i][0] = bond0min;
        seq_box[i][1] = bond0max;
        seq_box[i][2] = bond1min;
        seq_box[i][3] = bond1max;
        seq_box[i][4] = bond2min;
        seq_box[i][5] = bond2max;                
//            cout<<" "<<tmp_i<<" "<<endl;                                       
    }
    ...
    for(int i=0;i<pnum;i++)
    {
        ...
        for(int x=floor(seq_box[i][0]);x<=ceil(seq_box[i][1]);x++)
        {
            for(int y=floor(seq_box[i][2]);y<=ceil(seq_box[i][3]);y++)
            {
                for(int z=floor(seq_box[i][4]);z<=ceil(seq_box[i][5]);z++)
                {
                    clc_x2 = rhoC(x,y,z);
    //                clc_x2 = tmp_rhc;
                    obs_x2 = density(x,y,z);
    //                obs_x2 = tmp_den;
                    eps_x2 = 1-inv_rho_mask0(x,y,z); //1/(1+exp( (0.01-rho_calc(x,y,z)) * 1000 ));  // sigmoidal
                //    eps_x2 = 1.0;
        ...
        CC_res.push_back(CC_i2);        
    }
    float CC_i4=0.0;
    vector<int> CC_v(pnum,0);
    int beg_i=0;
    int end_i=0;
    int num = 0;
    float CC_m = 0.0;
...
/*    float CC_mx =0.0;
    for(int i=0;i<pp_t;i++)
    {
        float CC_t = CC_res[i];
        if(CC_t>0.0)
        {
            CC_mx = CC_mx + CC_v[i];
//          float CC_g = 1.0/(1.0+1.0/(CC_v[i]));
//          float CC_g = 1.0/(1.0+ (1.0/(float (CC_v[i])*CC_res[i]))); //*(1.0/(CC_v[i]*CC_res[i]));
//          CC_m = CC_m + CC_g;
        }
    }   */
//  float d0= float(pp_t)/CC_mx;
    CC_m =0.0;
    for(int i=0;i<pnum;i++)
    {
        float CC_t = CC_res[i];
        if(CC_t>0.0)
        {
//          CC_m = CC_m + CC_v[i]*CC_res[i];
//          float CC_g = 1.0/(1.0+1.0/(CC_v[i]));
//          float CC_g = CC_mx/(1.0+ (1.0/(float (CC_v[i])))); //*(1.0/(CC_v[i]*CC_res[i]));// 9.97
            float CC_g = 1.0/(1.0+ 1.0/(float (CC_v[i]))+ 1.0/CC_res[i]);
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
            CC_m = CC_m + CC_g;
        }
    }
    CC_i4 = CC_m/float(pnum);
    CC_res3.push_back(CC_i4);
...
//          float CC_g = 1.0/(1.0+1.0/(CC_v[i]));
//          float CC_g = CC_mx/(1.0+ (1.0/(float (CC_v[i])))); //*(1.0/(CC_v[i]*CC_res[i]));// 9.97
//          float CC_j = 5.0/float (CC_v[i]);
//          float CC_g = CC_res[i]/(1.0+ CC_j*CC_j);
            float CC_h = 1.0/float(pnum*CC_t);
//          float CC_h = d0/float(num_CC);
//          float CC_h = d0/(CC_t);
            float CC_g = 1.0/(1.0 + CC_h);
//          float pk= 1.5* sqrt(float(pp_t))+ 5.0;
//          float CC_h = float(CC_v[i])/float(pp_t);
//          float CC_h =(1.0 + float(CC_v[i])/float(pp_t));
//          float CC_g = CC_t*(CC_h * CC_h);
//          float CC_h =(1.0 + float(CC_v[i])/float(pp_t));
//          float CC_g = 1.0/CC_t+float(pp_t)/float(CC_v[i]);           
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
            CC_m = CC_m + CC_g;
        }
    }
    CC_i5 = CC_m/float(pnum);    
    CC_res3.push_back(CC_i5);

    float sumC_i3=0.0, sumO_i3=0.0, sumCO_i3=0.0, vol_i3=0.0, CC_i3=0.0;
    float sumO2_i3=0.0, sumC2_i3=0.0, varC_i3=0.0, varO_i3=0.0;
    float clc_x3=0.0, obs_x3=0.0, eps_x3=0.0;
    for ( int x=0; x<density.u1()*density.u2()*density.u3(); ++x ) {
//                tmp_den = 0.0; 
//                tmp_rhc=0.0;
//              if(theDensityMap.density[x]>(2.0/3.0)*max_den) tmp_den=theDensityMap.density[x];
...
//                clc_x2 = tmp_rhc;
        obs_x3 = density[x];
//                obs_x2 = tmp_den;
        eps_x3 = 1-inv_rho_mask0[x]; //1/(1+exp( (0.01-rho_calc(x,y,z)) * 1000 ));  // sigmoidal
//        eps_x2 = 1.0;

        // SMOOTHED
        sumCO_i3 += eps_x3*clc_x3*obs_x3;
        sumO_i3  += eps_x3*obs_x3;
        sumO2_i3 += eps_x3*obs_x3*obs_x3;
        sumC_i3  += eps_x3*clc_x3;
        sumC2_i3 += eps_x3*clc_x3*clc_x3;
        vol_i3   += eps_x3;
    }
    varC_i3 = (sumC2_i3 - sumC_i3*sumC_i3 / vol_i3 );
    varO_i3 = (sumO2_i3 - sumO_i3*sumO_i3 / vol_i3 ) ;
    if ( varC_i3 == 0 || varO_i3 == 0 ) {
        CC_i3 = 0;
    } else {
        CC_i3 = (sumCO_i3 - sumC_i3*sumO_i3/ vol_i3) / sqrt( varC_i3 * varO_i3 );
    }

    CC_res3.push_back(CC_i3);

    return CC_res3;        
}

vector<float> ElectronDensity::matchposex_x(vector<poseCoord> PB, int seq_num)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
...
vector<float> ElectronDensity::matchposet_x(vector<poseCoord> PB)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
    ...
    int Denz = density.u3();
    int Denxyz = Denx*Deny*Denz;
    rhoC.dimension(Denx , Deny , Denz,0.0);
    inv_rho_mask0.dimension(Denx , Deny , Denz,1.0);
    ObjexxFCL::FArray3D< float > rhoCg;
    ObjexxFCL::FArray3D< float > inv_rho_maskg;  
    vector<float> CC_res; 
    int pp_t=0; 
    vector<float> CC_res3;
...
//        cout<<"YYYY: "<<pointsBx[i].elt_<<endl;;
        if(PB[i].elt_ == "CA")
        {
            float bond0max=-10000.0,bond1max=-10000.0,bond2max=-10000.0;
            float bond0min=10000.0,bond1min=10000.0,bond2min=10000.0; 
            rhoCg.dimension(Denx , Deny , Denz,0.0);
            inv_rho_maskg.dimension(Denx , Deny , Denz,1.0);
               ...
               for(int y=int(bond1min);y<=bond1max;y++)
                {
                    for(int z=int(bond2min);z<=bond2max;z++)
                    {
                        ...
                        clc_x3 = rhoCg(x,y,z);
        //                clc_x2 = tmp_rhc;
                        obs_x3 = density(x,y,z);
        //                obs_x2 = tmp_den;
                        eps_x3 = 1-inv_rho_maskg(x,y,z); //1/(1+exp( (0.01-rho_calc(x,y,z)) * 1000 ));  // sigmoidal
                    //    eps_x2 = 1.0;
            ...
            varC_i3 = (sumC2_i3 - sumC_i3*sumC_i3 / vol_i3 );
            varO_i3 = (sumO2_i3 - sumO_i3*sumO_i3 / vol_i3 ) ;
            if ( varC_i3 == 0 || varO_i3 == 0 || vol_i3==0) {
                CC_i3 = 0;
            } else {
                CC_i3 = (sumCO_i3 - sumC_i3*sumO_i3/ vol_i3) / sqrt( varC_i3 * varO_i3 );
            } 
            CC_res.push_back(CC_i3);
            pp_t = pp_t + 1; 
        }       
//            cout<<" "<<tmp_i<<" "<<endl;                                       
    }
    float CC_i4=0.0;
    vector<int> CC_v(pp_t,0);
    int beg_i=0;
    int end_i=0;
    int num = 0;
    float CC_m = 0.0;
    vector<float> CC_resg(pp_t,0);
    for(int i=0;i<pp_t;i++)
    {
       int j_0=i-1;
       int j_1=i+1;
       if(j_0<0) j_0=0;
       if(j_1>(pp_t)) j_1=pp_t-1;
       CC_resg[i]=(CC_res[j_0]+CC_res[i]+CC_res[j_1])/3.0;
    }
    for(int i=0;i<pp_t;i++)
    {
        float CC_t = CC_res[i];
...
//  float d0= float(pp_t)/CC_mx;
    ...
    for(int i=0;i<pp_t;i++)
    {
        float CC_t = CC_resg[i];
        if(CC_t>0.0)
        {
//          CC_m = CC_m + CC_v[i]*CC_res[i];
//          float CC_g = 1.0/(1.0+1.0/(CC_v[i]));
//          float CC_g = CC_mx/(1.0+ (1.0/(float (CC_v[i])))); //*(1.0/(CC_v[i]*CC_res[i]));// 9.97
//            float CC_g = float(1.0+1.0/float(pp_t))*1.0/(1.0+ 1.0/(float (CC_v[i])));//+ 1.0/CC_res[i]);i
            float CC_g = 1.0/(1.0+ float(pp_t)/(float (CC_v[i])));//+ 1.0/CC_res[i]);
//	    float CC_g = 1.0/(1.0+ float(pp_t)/(float (CC_v[i])));//+ 1.0/CC_res[i]);
//	    float CC_g = 1.0/(1.0+ 1.0/(float (CC_v[i])*CC_res[i]));
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
            CC_m = CC_m + CC_g;
            pp_j=pp_j+1;
        }
    }
    CC_i4 = 2.0*float(pp_j)*CC_m/float(pp_t*pp_t);
    CC_i4=sqrt(CC_i4);
    CC_res3.push_back(CC_i4);

    float d0=pow(1.0/float(pp_t),float(1.0/3.0));
    float CC_i5=0.0;
    CC_m =0.0;
    int pp_n=0;
    for(int i=0;i<pp_t;i++)
...
//          float CC_j = 5.0/float (CC_v[i]);
//          float CC_g = CC_res[i]/(1.0+ CC_j*CC_j);
//            float CC_h = 1.0/float(pp_t*CC_t);
	  float CC_h = d0/float(CC_t);
//          float CC_h = d0/float(num_CC);
//          float CC_h = d0/(CC_t);
//            float CC_g = (1.0+d0*d0)*1.0/(1.0 + CC_h*CC_h);
          float CC_g = 1.0/(1.0 + CC_h);
//          float pk= 1.5* sqrt(float(pp_t))+ 5.0;
//          float CC_h = float(CC_v[i])/float(pp_t);
//          float CC_h =(1.0 + float(CC_v[i])/float(pp_t));
...
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
    ...
    CC_i5 = (1.0+d0)*float(pp_n)*CC_m/float(pp_t*pp_t); 
    CC_i5 =sqrt(CC_i5);   
    CC_res3.push_back(CC_i5);   
...
//        eps_x2 = 1.0;
    ...
    CC_res3.push_back(CC_i2);
    return CC_res3;        
}

vector<float> ElectronDensity::matchposet_y(vector<poseCoord> PB)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
...
//        if(PB[i].elt_ == "CA")
    ...
    //        cout<<"RRRR: "<<atm_idx[tmp_i][0]<<" "<<atm_idx[tmp_i][1]<<" "<<atm_idx[tmp_i][2]<<endl; 
            for(int z=1;z<=Denz;z++)
            {
                ...
                MatrixTimesTransVector(f2c,del_ijx,frac_tmpz);
//                if(square_len(frac_tmpz)> (ATOM_MASK_PADDING+CA_MASK)*(ATOM_MASK_PADDING+CA_MASK) ) continue;                    
                if(square_len(frac_tmpz)> 30.0 ) continue;
                if(float(z)>bond2max) bond2max = float(z);  
                if(float(z)<bond2min) bond2min = float(z);                  
                for(int y=1;y<=Deny;y++)
                {
                    ...
                    MatrixTimesTransVector(f2c,del_ijx,frac_tmpy);
//                    if(square_len(frac_tmpy)> (ATOM_MASK_PADDING+CA_MASK)*(ATOM_MASK_PADDING+CA_MASK) ) continue;    
                    if(square_len(frac_tmpy)> 30.0 ) continue;                    
                    if(float(y)>bond1max) bond1max = float(y);  
                    if(float(y)<bond1min) bond1min = float(y);                        
                    for(int x=1;x<=Denx;x++)
                    {
...
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
    ...
    CC_i4 =float(pp_n)*2.0*CC_m/float(pp_t*pp_t);
    CC_i4 = sqrt(CC_i4);
    CC_res3.push_back(CC_i4);
...
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
    ...
    CC_i5 = float(pp_j)*(1.0+d0)*CC_m/float(pp_t*pp_t);   
    CC_i5=sqrt(CC_i5); 
    CC_res3.push_back(CC_i5);   
...
float ElectronDensity::matchposet(vector<poseCoord> PB)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
...
float ElectronDensity::matchposey(poseCoord PB)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
...
//        if ( del_ijx[2] < -0.5 ) del_ijx[2]+=1.0;                    
        ...
        vector<float> frac_tmpz;
        MatrixTimesTransVector(f2c,del_ijx,frac_tmpz);
        if(square_len(frac_tmpz)> (ATOM_MASK_PADDING+ATOM_MASK)*(ATOM_MASK_PADDING+ATOM_MASK) ) continue;  
        if(float(z)>bond2max) bond2max = float(z);  
        if(float(z)<bond2min) bond2min = float(z);                
        for(int y=1;y<=density.u2();y++)
        {
...
//            if ( del_ijx[1] < -0.5 ) del_ijx[1]+=1.0;                        
            ...
            vector<float> frac_tmpy;
            MatrixTimesTransVector(f2c,del_ijx,frac_tmpy);
            if(square_len(frac_tmpy)> (ATOM_MASK_PADDING+ATOM_MASK)*(ATOM_MASK_PADDING+ATOM_MASK) ) continue;
            if(float(y)>bond1max) bond1max = float(y);  
            if(float(y)<bond1min) bond1min = float(y);
            for(int x=1;x<=density.u1();x++)
            {
...
//                if ( del_ijx[0] < -0.5 ) del_ijx[0]+=1.0;                            
                ...
                MatrixTimesTransVector(f2c,del_ijx,cart_del_ij2);
                float d2 = square_len(cart_del_ij2);
                if(d2 > (ATOM_MASK_PADDING+ATOM_MASK)*(ATOM_MASK_PADDING+ATOM_MASK) ) continue;
...
//    sumC2_i2 = sumC2_i2 / vol_i2;
    ...
    CC_i2 = sumCO_i2/(sqrt(sumO2_i2 + sumC2_i2));    */
    for(int x=int(bond0min);x<=bond0max;x++)
    {
       for(int y=int(bond1min);y<=bond1max;y++)
        {
...
float ElectronDensity::matchposez(vector<poseCoord> PBx)
{
	int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
//		}
//	}	
//	CC_ave = CC_ave/float(tmpt);
//	for(int i=0;i<pp_t;i++)
//	{
//		float CC_t = CC_res[i];
//		if(CC_t>0.0)
//		{
//			CC_var = CC_var + (CC_t-CC_ave)*(CC_t-CC_ave);
//		}
		
//	}
//	CC_var = sqrt(CC_var/(tmpt-1));
//	for(int i=0;i<pp_t;i++)
//	{
//		CC_i3 = CC_i3 + (CC_res[i]-CC_ave)/CC_var; 
//	}   

//	float d0= 0.42-0.05*float(pp_t)*exp(-float(pp_t)/4.7)+0.63*exp(-float(pp_t)/37.0);
//	float d0 = 1.24*pow(float(pp_t-15),1.0/3.0)-1.8;
//	float d0=pow(1.0/float(pp_t),1.0/3.0);
	float CC_m =0.0;
	for(int i=0;i<pp_t;i++)
	{
		float CC_t = CC_res[i];
...
//			float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
	...
	CC_i3 = CC_m/float(pp_t); 
    return CC_i3;        
} 
float ElectronDensity::matchposeR(vector<poseCoord> PBx,vector<boneinfo> bb)
{
    int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
//  float d0= float(pp_t)/CC_mx;
    ...
    {
        float CC_t = CC_res[i];
	if(bb[i].sst==0) continue;
        if(CC_t>0.0)
        {
//          CC_m = CC_m + CC_v[i]*CC_res[i];
//          float CC_g = 1.0/(1.0+1.0/(CC_v[i]));
//          float CC_g = CC_mx/(1.0+ (1.0/(float (CC_v[i])))); //*(1.0/(CC_v[i]*CC_res[i]));// 9.97
            float CC_b = 1.0/float(CC_v[i]*CC_res[i]);
            float CC_g = 1.0/(1.0 + sqrt(CC_b));
//            float CC_g = 1.0/(1.0+ 1.0/(float (CC_v[i]))+ 1.0/CC_res[i]);
//          float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
            CC_m = CC_m + CC_g;
        }
...
float ElectronDensity::matchposem(vector<poseCoord> PBx)
{
	int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
float ElectronDensity::matchposen(vector<poseCoord> PBx)
{
	int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
float ElectronDensity::matchposeq(vector<poseCoord> PBx)
{
	int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
float ElectronDensity::matchposep(vector<poseCoord> PBx)
{
	int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
//        cout<<"RRRR: "<<atm_idx[tmp_i][0]<<" "<<atm_idx[tmp_i][1]<<" "<<atm_idx[tmp_i][2]<<endl; 
        for(int z=1;z<=Denz;z++)
        {
            ...
            vector<float> frac_tmpz;
            MatrixTimesTransVector(f2c,del_ijx,frac_tmpz);
            if(square_len(frac_tmpz)> (6.0)*(6.0) ) continue;                    
            for(int y=1;y<=Deny;y++)
            {
                atm_jx[1] = y;
                del_ijx[1] = (atm_idx[1] - atm_jx[1])/grid[1];                      
                ...
                vector<float> frac_tmpy;
                MatrixTimesTransVector(f2c,del_ijx,frac_tmpy);
                if(square_len(frac_tmpy)> (6.0)*(6.0) ) continue;                        
                for(int x=1;x<=Denx;x++)
                {
                    atm_jx[0] = x;
                    del_ijx[0] = (atm_idx[0] - atm_jx[0])/grid[0];                         
                    ...
                    MatrixTimesTransVector(f2c,del_ijx,cart_del_ij2);
                    float d2 = square_len(cart_del_ij2);
                    if(d2 > (6.0)*(6.0) ) continue;
                
                    float atm = C*exp(-k*d2);
//                            cout<<"MASK: "<<theDensityMap.ATOM_MASK<<endl;
                    float sigmoid_msk = exp( d2 - (6.0)*(6.0)  );
                //    float sigmoid_msk = exp( sqrt(d2) - (theDensityMap.ATOM_MASK)  );
                //    float sigmoid_msk = exp( sqrt(d2) - (theDensityMap.CA_MASK)  );
                    float inv_msk = 1/(1+sigmoid_msk);
                    rhoC(x,y,z) += atm;
 ...
 */
vector<float> ElectronDensity::matchposeg(vector<point3f> PBx)
{
	int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
float ElectronDensity::matchposew(vector<point3f> PBx)
{
    int pp_y = PBx.size();
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
...
//	utility::vector1< std::string > symList;
	...
	density_filter.dimension( vol_xsize,vol_ysize,vol_zsize,0.0);
...
//	cout<<"rowdata: "<<rowdata[32]<<endl;
	...
	for(int x=1;x<=density.u1();x++)
	{
		for(int y=1;y<=density.u2();y++)
		{
			for(int z=1;z<=density.u3();z++)
			{
				float tmp_p = float(density(x,y,z)-min_den)/float(max_den-min_den);
				density(x,y,z) = tmp_p;
//				if(tmp_p > cut_denx)
//				{
					density_filter(x,y,z) = tmp_p;
//				}
			}
		}
	}

    vector<std::pair< vector< int >, float > > point_score_pairs;
    for ( int z=1; z<=(int)density.u3(); z++ ) {
        for ( int y=1; y<=(int)density.u2(); y++ ) {
            ...
            //  numeric::xyzVector< core::Real > x_idx(x,y,z);
                vector<int> x_idx(3,0.0);
                x_idx[0] = x;
                x_idx[1] = y;
                x_idx[2] = z;
                float dens_value = density(x,y,z);
                std::pair< vector<int>, float> point_score_pair = std::make_pair(x_idx, dens_value);
                point_score_pairs.push_back(point_score_pair);
            }
        }
    }
    std::sort(point_score_pairs.begin(), point_score_pairs.end(), PointScoreComparatory());
    int pre1 = int (density.u1()*density.u2()*density.u3()*0.09);
    int pre2 = int (density.u1()*density.u2()*density.u3()*0.05);
    float cut_deny = point_score_pairs[pre1].second;// cut off 
    float cut_deny1 = point_score_pairs[pre2].second;// cut off
    for(int x=1;x<=density.u1();x++)
    {
        for(int y=1;y<=density.u2();y++)
        {
...
//                float tmp_p = float(density(x,y,z)-min_den)/float(max_den-min_den);
//                density(x,y,z) = tmp_p;
                if(density(x,y,z) < cut_deny)
                {
//                    density_filter(x,y,z) = tmp_p;
                    density(x,y,z) = 0.0;
                } 
                if(density_filter(x,y,z)<cut_deny1)
                {
                   density_filter(x,y,z) = 0.0;
                }
            }
        }
    }
...
/*	cellDimensions[0] = cellDimensions[0] /this->grid[0];
	...
	cout<<"c2f: "<<c2f[0][0]<<" "<<c2f[0][1]<<" "<<c2f[0][2]<<endl;
	cout<<"     "<<c2f[1][0]<<" "<<c2f[1][1]<<" "<<c2f[1][2]<<endl;
	cout<<"     "<<c2f[2][0]<<" "<<c2f[2][1]<<" "<<c2f[2][2]<<endl;
	cout<<"f2c: "<<f2c[0][0]<<" "<<f2c[0][1]<<" "<<f2c[0][2]<<endl;
	cout<<"     "<<f2c[1][0]<<" "<<f2c[1][1]<<" "<<f2c[1][2]<<endl;
	cout<<"     "<<f2c[2][0]<<" "<<f2c[2][1]<<" "<<f2c[2][2]<<endl;	

//	cout<<"altorigin: "<<altorigin[0]<<" "<<altorigin[1]<<" "<<altorigin[2]<<endl;
...
//	}
	...
	this->max_del_grid = max_del_grid;
