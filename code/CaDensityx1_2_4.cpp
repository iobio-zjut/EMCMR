...
float ElectronDensity::matchposedt(vector<poseCoord> PB, vector<distmap> dist_map,int seq_num)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
...
//            cout<<" "<<tmp_i<<" "<<endl;                                       
    ...
    for(int iu=0;iu<N_dist;iu++)
    {
        ...
        CC_i4 = CC_i4 + ((CC_res[ik]+CC_res[jk]) + 0.01)/(abs(CC_res[ik]-CC_res[jk]) + 0.01);
//        float tmp_x = dist_map[iu].dv;
//        float tmp_y = Distance_point(fin_maty[3*ik+1].x_,fin_maty[3*jk+1].x_);
//        E_distx = E_distx + sqrt((tmp_x-tmp_y)*(tmp_x-tmp_y));

    }    
    CC_i4 = CC_i4/float(N_dist); 
...
float ElectronDensity::matchpose_lpl(vector<poseCoord> PB, int seq_num)
{
    vector<float> del_ijx(3,0.0);
    vector<float> atm_jx(3,0.0); 
    string elt_i;   
...
//    {
        ObjexxFCL::FArray3D< double > density_lplx;
        density_lplx.dimension(Denx, Deny, Denz);
        for(int x=1;x<=Denx;x++)
        {
            int x1 = x+1,x_1 = x-1;
            if(x_1<1) x_1 = 1;
            if(x1>int(Denx)) x1 = int(Denx);
            for(int y=1;y<=Deny;y++)
            {
                int y1 = y+1,y_1 = y-1;
                if(y_1<1) y_1 = 1;
                if(y1>int(Deny)) y1 = int(Deny);
                for(int z=1;z<=Denz;z++)
                {
                    int z1 = z+1,z_1 = z-1;
                    if(z_1<1) z_1 = 1;
                    if(z1>int(Denz)) z1 = int(Denz);
                    density_lplx(x,y,z) = double(-6.0*rhoC(x,y,z) + rhoC(x1,y,z) + rhoC(x_1,y,z)+ rhoC(x,y1,z)+ rhoC(x,y_1,z)+ rhoC(x,y,z1)+ rhoC(x,y,z_1));
                }
            }
        }
//        fourier::fft3(density_lplx, FrhoC0); 
//        rhoC0.clear();
//        rhoC0 = density_lplx;
//    }
    ...
    for(int i=0;i<pnum;i++)
    {
        ...
        for(int x=floor(seq_box[i][0]);x<=ceil(seq_box[i][1]);x++)
        {
            for(int y=floor(seq_box[i][2]);y<=ceil(seq_box[i][3]);y++)
            {
                for(int z=floor(seq_box[i][4]);z<=ceil(seq_box[i][5]);z++)
                {
                    clc_x2 = density_lplx(x,y,z);
    //                clc_x2 = tmp_rhc;
                    obs_x2 = density_lpl(x,y,z);
    //                obs_x2 = tmp_den;
                    eps_x2 = 1-inv_rho_mask0(x,y,z); //1/(1+exp( (0.01-rho_calc(x,y,z)) * 1000 ));  // sigmoidal
                //    eps_x2 = 1.0;
...
//			float CC_g = 1.0/(1.0+1.0/(CC_v[i]));
//			float CC_g = CC_mx/(1.0+ (1.0/(float (CC_v[i])))); //*(1.0/(CC_v[i]*CC_res[i]));// 9.97
			float CC_g = 1.0/(1.0+ 1.0/(float (CC_v[i]))+ 1.0/(CC_res[i]*CC_res[i]));
//			float CC_g = 1.0/(1.0+ (d0/(float (CC_res[i])))); //*(1.0/(CC_v[i]*CC_res[i]));
			CC_m = CC_m + CC_g;
		}
	}
    ...
    for(int x=1;x<=density.u1();x++)
    {
        ...
        for(int y=1;y<=density.u2();y++)
        {
            ...
            for(int z=1;z<=density.u3();z++)
            {
                ...
                density_lpl(x,y,z) = double(-6.0*density(x,y,z) + density(x1,y,z) + density(x_1,y,z)+ density(x,y1,z)+ density(x,y_1,z)+ density(x,y,z1)+ density(x,y,z_1));
            }
        }
    }
//	cout<<"ATOM_MASK: "<<ATOM_MASK<<endl;
