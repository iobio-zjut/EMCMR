...
//				cout<<"old_dE: "<<old_dE<<endl;	
			...
			//	GroupRotationy(tmp_mat[0].ca,tmp_mat[tmp_mat.size()-1].ca,angle_rotate,tmp_mat);
				...
				{
					float VR1= GetVdwRadius(tmp_mat[j].elt_);
					for(int t=0;t<pointsBx.size();t++)
					{
						float VR2= GetVdwRadius(pointsBx[t].elt_);
						dis_p = Distance_point(tmp_mat[j].x_,pointsBx[t].x_);
						if((VR1+VR2)<(1.2*dis_p)) tp_clash = true;
					}
				}
				if(tp_clash) continue;
...
///	writePDBcoordss(inputPDB1, fin_p);
	...
	if(tmp_strx.size()>0)
	{
		outname = tmp_strx[tmp_strx.size()-1] + "_outp.pdb";
	}
	else
	{
		outname = tmp_strx[0] + "_outp.pdb";
	}
