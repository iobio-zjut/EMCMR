...
//		axyz[2]=pointsABx[rand_point][2];
		axyz[0] = pointsBx[rand_point].x_[0];
		axyz[1] = pointsBx[rand_point].x_[1];
		axyz[2] = pointsBx[rand_point].x_[2];		

//		theDensityMap.calcRhoC(pointsBx,4.0,rhoC,rhoMask);
//		old_dE=theDensityMap.getRSCCX(rhoC,rhoMask);
//		cout<<"old_dE: "<<old_dE<<endl;	
		old_dE1 =0.0;
		vector<poseCoord> pB1,pB2;		
	  	for(int j=0;j<rand_point;j++)
	  	{
	  		pB1.push_back(pointsBx[j]);
//	  		old_dE1 = old_dE1 + Distance_point(pointsABx[j],pointsABy[j]);
	  	}
		theDensityMap.calcRhoC(pB1,4.0,rhoC,rhoMask);
		old_dE1=theDensityMap.getRSCCX(rhoC,rhoMask);
		cout<<"old_dE1: "<<old_dE1<<endl;	
	  	old_dE2 =0.0;
	  	for(int j=rand_point;j<pnumx;j++)
	  	{
	  		pB2.push_back(pointsBx[j]);
//	  		old_dE2 = old_dE2 + Distance_point(pointsABx[j],pointsABy[j]);
	  	}		
		theDensityMap.calcRhoC(pB2,4.0,rhoC,rhoMask);
		old_dE2=theDensityMap.getRSCCX(rhoC,rhoMask);
		cout<<"old_dE2: "<<old_dE2<<endl;		  	  			
...
//			vector<vector<float> >(0,vector<float>()).swap(fin_mat);
			...
			int tmp3 =0;
	  		for(int j=rand_point;j<pnumx;j++)
	  		{
				fin_mat[tmp3].x_[0]=t1+axyz[0]+(pointsABx[j].x_[0]-axyz[0])*u[0][0]+(pointsABx[j].x_[1]-axyz[1])*u[0][1]+(pointsABx[j].x_[2]-axyz[2])*u[0][2];
				fin_mat[tmp3].x_[1]=t2+axyz[1]+(pointsABx[j].x_[0]-axyz[0])*u[1][0]+(pointsABx[j].x_[1]-axyz[1])*u[1][1]+(pointsABx[j].x_[2]-axyz[2])*u[1][2];
				fin_mat[tmp3].x_[2]=t3+axyz[2]+(pointsABx[j].x_[0]-axyz[0])*u[2][0]+(pointsABx[j].x_[1]-axyz[1])*u[2][1]+(pointsABx[j].x_[2]-axyz[2])*u[2][2];	  			
	  			tmp3= tmp3 +1 ;
	  		}
//			new_dE2 = 0.0;
  //			int tmp1=0;
...
//				min_dE[i] = Distance_point(pointsABx[i],pointsABy[i]);//pointsx[i].ca[0] - pointsy[i].ca[0] 	
				...
				int rand_bx=0;
		  		...
		  		pBx.clear();
		  		for(int j=rand_ax;j<rand_bx;j++)
		  		{
		  			pBx.push_back(pointsBx[j]);
		  		}						
				theDensityMap.calcRhoC(pBx,4.0,rhoC,rhoMask);
//				min_dE[i]=theDensityMap.getRSCC(rhoC,rhoMask);
				min_dE.push_back(theDensityMap.getRSCC(rhoC,rhoMask));
//				cout<<i<<" min_dE: "<<min_dE[i]<<endl;	
				if(min_dE[tpm]>max_ca)
				{
					max_ca = min_dE[tpm];
					max_ca_index = ii;
				}
				if(min_dE[tpm]<min_ca)
				{
					min_ca = min_dE[tpm];
					min_ca_index = ii;
				}
		//		cout<< min_dE[i]<<" ";	
			...
			if(min_ca_index<0) randp =rand()%(pnumx);
			else randp = min_ca_index;  */
//			cout<<"ttt: "<<ttt<<endl;
			randp = rand()%(pnumx);
		  	for(int i=0;i<200;i++)
		  	{
...
//					old_dE = old_dE + Distance_point(pointsABx[j],pointsABy[j]);
				...
				theDensityMap.calcRhoC(tmp_mat,4.0,rhoC,rhoMask);
				old_dE = theDensityMap.getRSCC(rhoC,rhoMask);
//				old_dE = theDensityMap.getRSCCX(rhoC,rhoMask);
//				cout<<"old_dE: "<<old_dE<<endl;	
...
//				cout<<"i: "<< i	<<endl;
				...
				for(int j=rand_a;j<=rand_b;j++)
				{
					float VR1= GetVdwRadius(tmp_mat[tp_ch].elt_);
					for(int t=0;t<pointsBx.size();t++)
					{
						if(t==j) continue;
						float VR2= GetVdwRadius(pointsBx[t].elt_);
						dis_p = Distance_point(tmp_mat[tp_ch].x_,pointsBx[t].x_);
						float VR = VR1 + VR2;
//						cout<<"dis_p "<<dis_p<<endl;
//						cout<<"VR1+VR2 "<<(VR1+VR2)<<endl;
						if(dis_p < VR )
						{
							tp_c = 2.0;
//							cout<< "dis_p: "<<dis_p<<endl;
...
//						cout<< "tp: "<<tp_c<<endl;
				...
				cout<<" tp_clash: "<< tp_c<<endl;
				if(tp_c > tp_cx) continue;  
//				cout<<"i: "<< i	<< " tp_clash"<<endl;		
/*				int randxx = rand()%(rand_b-rand_a+1)+rand_a;
			...
			//		if(dis_s0>1.32169 && dis_s0<1.33883 && dis_s1<(1.53698) && dis_s1>(1.51328)) tmp_mat[randyy] = tmpcc;
				...
				new_dE = 0.0;
				theDensityMap.calcRhoC(tmp_mat,4.0,rhoC,rhoMask);
				new_dE = theDensityMap.getRSCC(rhoC,rhoMask);
//				new_dE = theDensityMap.getRSCCX(rhoC,rhoMask);
...
//						cout<<" atmnmy,CBp: "<<atmnmy<<" "<<CBp[j].Rotnm[tj]<<endl;					
				//		if(atmnmy==CBp[j].Rotnm[tj]) {fin_pose.chains[i].residues[j].atoms[t].xyzVect = CBp[j].Rotcd[tj]; break;}
					...
					fin_pose.chains[i].residues[j].atoms[t].xyzVect[0] = fin_pose.chains[i].residues[j].atoms[t].xyzVect[0] - pointsBx0[3*j+1].x_[0]+pointsBx[3*j+1].x_[0];
					fin_pose.chains[i].residues[j].atoms[t].xyzVect[1] = fin_pose.chains[i].residues[j].atoms[t].xyzVect[1] - pointsBx0[3*j+1].x_[1]+pointsBx[3*j+1].x_[1];
					fin_pose.chains[i].residues[j].atoms[t].xyzVect[2] = fin_pose.chains[i].residues[j].atoms[t].xyzVect[2] - pointsBx0[3*j+1].x_[2]+pointsBx[3*j+1].x_[2];
					tt=tt+1;
				}									
				if(Atmx.atona == " CA ") 
				{
					fin_pose.chains[i].residues[j].atoms[t].xyzVect = pointsBx[3*j+1].x_;
...
///	writePDBcoordss(inputPDB1, fin_p);
	...
	if(tmp_strx.size()>0)
	{
		outname = tmp_strx[tmp_strx.size()-1] + "_outp1.pdb";
	}
	else
	{
		outname = tmp_strx[0] + "_outp1.pdb";
	}
