...
OneGaussianScattering get_A( std::string elt, bool cryoem_scatterers ) {
	static std::map< std::string, OneGaussianScattering > elt_db;
	float centroid_density_mass=0.0;
	...
	if ( elt_db.find( elt ) == elt_db.end() ) {
		// default to C
		std::cout << "Unknown atom " << elt << std::endl;
		return elt_db["C"];
	} else {
		return elt_db[ elt ];
	}
