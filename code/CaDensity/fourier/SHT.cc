...
#include <fourier/kiss_fft.hh>
#include <fourier/SHT.hh>
...
void SHT::sharm_transform(
	...
	vector< double > sigRslice( 2*bw*2*bw );
	vector< double > sigCoefRslice( bw*bw );
	vector< double > sigCoefIslice( bw*bw );
...
)
{
	...
	for ( int r_idx=1; r_idx<=R; ++r_idx ) {
		// standardize
		double thisRWt = PI * ( 4.0 * square((double)r_idx) + 1.0/3.0 );
...
void SHT::so3_correlate(
	...
	vector< double > sigCoefR_slice(bw*bw);
	vector< double > sigCoefI_slice(bw*bw);
	vector< double > tmpCoefR_slice(bw*bw);
	vector< double > tmpCoefI_slice(bw*bw);
...
void SHT::idx_to_rot(int maxloc , vector<vector<double>> & thisRot) {
	// uses current BW
	if ( bw == 0 ) {
		thisRot = vector<vector<double>> (3,vector<double>(3,0.0));
		thisRot[0][0]=1.0 ;
		thisRot[1][1]=1.0 ;
		thisRot[2][2]=1.0 ;
	...
	double a = PI*kk/((double) bw);
	double b = PI*(2*ii+1)/(4.0*bw);
	double g = PI*jj/((double) bw);

	thisRot[0][0] = (- sin(a)*cos(b)*sin(g) + cos(a)*cos(g));
	thisRot[0][1] = (cos(a)*cos(b)*sin(g) + sin(a)*cos(g));
	thisRot[0][2] = (sin(b)*sin(g));
	thisRot[1][0] = (-sin(a)*cos(b)*cos(g) - cos(a)*sin(g));
	thisRot[1][1] = (cos(a)*cos(b)*cos(g) - sin(a)*sin(g));
	thisRot[1][2] = (sin(b)*cos(g));
	thisRot[2][0] = (sin(a)*sin(b));
	thisRot[2][1] = (-cos(a)*sin(b));
	thisRot[2][2] = (cos(b));
...
void SHT::idx_to_euler(int maxloc , vector< double > & eulerAngles) {
	if ( bw == 0 ) {
		eulerAngles = vector< double >(3,0);
		return;
	}
	...
	eulerAngles[0] = PI*kk/((double) bw);
	eulerAngles[1] = PI*(2*ii+1)/(4.0*bw);
	eulerAngles[2] = PI*jj/((double) bw);
}
...
SHT::so3CombineCoef(
	...
	for ( int l = 0 ; l < bw ; l ++ ) {
		double wigNorm = 2.*PI*sqrt(2./(2.*l+1.)) ;
...
SHT::setup_Pmls( ) {
	...
	// scratch space
	vector< double > prev(bw), prevprev(bw);
	vector< double > arccosres(bw), x_i(bw), eval_args(bw), cosres(bw);
	...
	for ( int m=0; m<bw; m++ ) {
		...
		for ( int i=0; i<bw; i++ ) {
			x_i[i] = cos((( 2.0*((double)i)+1.0 ) * PI) / (2.0*bw));
			eval_args[i] = (( 2.0*((double)i)+1.0 ) * PI) / (2.0*bw);
		}
...
SHT::setup_Wig( ) {
	...
	// scratch space
	vector< double > sinPts(N);
	vector< double > cosPts(N);
	vector< double > sinPts2(N);
	vector< double > cosPts2(N);
	vector< double > scratch1(N);
	vector< double > scratch2(N);

	//  precompute sines and cosines
	for ( int i=0; i<N; i++ ) {
		sinPts[i] = sin( (( 2.0*((double)i)+1.0 ) * PI) / (2.0*N));
		cosPts[i] = cos( (( 2.0*((double)i)+1.0 ) * PI) / (2.0*N));
		sinPts2[i] = sin( 0.5 * (( 2.0*((double)i)+1.0 ) * PI) / (2.0*N));
		cosPts2[i] = cos( 0.5 * (( 2.0*((double)i)+1.0 ) * PI) / (2.0*N));
	}
...
SHT::forwardS2(
	...
	// scratch space
	vector< double > rres(N*N);
	vector< double > ires(N*N);
	vector< double > fltres(bw);
	vector< double > eval_pts(N);
	vector< double > scratch1(N);
	vector< double > scratch2(N);
	vector< double > idata(N,0.0); //dummy
	...
	// normalize
	double scale = 1./ (2.0*bw) * sqrt( 2.0*PI );
	for ( int i = 0 ; i < N*N ; i ++ ) {
		rres[i] *= scale;
		ires[i] *= scale;
	}
...
SHT::inverseS2(
	...
	// scratch
	vector< double > rfourdata(N*N);
	vector< double > ifourdata(N*N);
	vector< double > rinvfltres(N);
	vector< double > iminvfltres(N);
	vector< double > sin_values(N);
	vector< double > scratch(N);

	// filler (real signal only)
	vector< double > idata(N*N, 0.0);

	for ( int i=0; i<N; i++ ) {
		sin_values[i] = sin( (( 2.0*((double)i)+1.0 ) * PI) / (2.0*N) );
	}
