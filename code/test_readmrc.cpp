...
//    rhoC01.dimension(theDensityMap.density.u1() , theDensityMap.density.u2() , theDensityMap.density.u3());
    ...
    ObjexxFCL::FArray3D< int > cal_mat;
    ObjexxFCL::FArray3D< int > den_mat;    
    cal_mat.dimension(theDensityMap.density.u1() , theDensityMap.density.u2() , theDensityMap.density.u3());
    den_mat.dimension(theDensityMap.density.u1() , theDensityMap.density.u2() , theDensityMap.density.u3());
//    inv_rho_mask1.dimension(theDensityMap.density.u1() , theDensityMap.density.u2() , theDensityMap.density.u3());
    for ( int t=0; t<theDensityMap.density.u1()*theDensityMap.density.u2()*theDensityMap.density.u3(); ++t ) {
        ...
        cal_mat[t]=0;
        den_mat[t]=0;
        theDensityMap.test_density[t] = 0;
    }
    cout<<"GGG"<<endl;
...
//            cartX1[0] = cartX1[0] - theDensityMap.altorigin[0];
//            cartX1[1] = cartX1[1] - theDensityMap.altorigin[1];
//            cartX1[2] = cartX1[2] - theDensityMap.altorigin[2];
...
//        cout<<theDensityMap.density[x]<<" ";
        ...
        if(theDensityMap.density[x] < min_den) min_den = theDensityMap.density[x] ;
        mean_den = theDensityMap.density[x];
        if(rhoC0[x]>max_rhc) max_rhc = rhoC0[x] ;
    } 
    mean_den = mean_den/(theDensityMap.density.u1()*theDensityMap.density.u2()*theDensityMap.density.u3());
    int den_bin = 200;
    float interval_bin = (max_den-min_den)/float(den_bin);
    vector<float> val_bin(den_bin+1);
    vector<int> prob_bin(den_bin,0);
    val_bin[0] = min_den;
    ...
    for(int x=0; x<theDensityMap.density.u1()*theDensityMap.density.u2()*theDensityMap.density.u3(); ++x ) 
    {
        float den_val = theDensityMap.density[x];
        for(int y=0;y<den_bin;y++)
        {
            if(den_val>=val_bin[y]&&den_val<val_bin[y+1]) 
            {
    ...
    for(int x=0;x<den_bin;x++)
    {
        if(prob_bin[x]>max_prob)
        {
            max_prob = prob_bin[x];
            max_prob_bin = x;
            den_cutoff = val_bin[x+1];
        }
    }
    cout<<"den_cutoff value: "<<den_cutoff<<endl;
    for(int x=1;x<=theDensityMap.density.u1();++x)
    {
       for(int y=1;y<=theDensityMap.density.u2();++y)
        {
            for(int z=1;z<=theDensityMap.density.u3();++z)
            {
                if(theDensityMap.density(x,y,z)>=den_cutoff) 
                {
                    den_mat(x,y,z) = 1 ;
...
//                theDensityMap.test_density(x,y,z) = theDensityMap.density(x,y,z);
    ...
    theDensityMap.writeMRC_test("xx4.mrc");
//    cout<<"YYY"<<endl;
